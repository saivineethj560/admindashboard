...
import { useNavigate, useParams } from 'react-router-dom';
 import { jsPDF } from 'jspdf';
  import logo from "../assets/imagesmy.png"
...
const HrmApprovals = () => {
 ...
 const navigate = useNavigate();
  ...
  const calculateDetailedBreakdown = (ctc) => {
    ...
    console.log(ctc,"ffffffffffffff");
...
console.log("saaaaaaaaaaaaaa",salary);
  ...
  const handlePreviewPDF = () => {
    ...
    try {
      ...
      doc.text('MY HOME GROUP', pw / 2, 7, { align: 'center' });
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text('SALARY BREAKUP DOCUMENT', pw / 2, 13, { align: 'center' });
      y = 20;

      const infoRows = [
        ['NAME', selectedData?.name],
        ['JOB TITLE',   selectedData?.DESIG || selectedData?.MANPOWER_DESG || 'N/A'],
        ['LOCATION', selectedData?.PLANT || 'N/A'],
      ...
      infoRows.forEach(([label, value], i) => {
        ...
        doc.rect(10, ry, pw - 20, 5, 'F');
        doc.rect(10, ry, pw - 20, 5, 'S');
        doc.setFontSize(7);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(80, 80, 80);
        doc.text(label, 14, ry + 3.4);
        doc.setFont('helvetica', 'normal');
      ...
      const row = (label, m, a, { isBold = false, isTotal = false, isHighlight = false, isDisabled = false, isFixed = false } = {}) => {
        doc.setFillColor(...(isHighlight ? [235, 245, 255] : isTotal ? [248, 250, 252] : ri % 2 === 0 ? [255, 255, 255] : [250, 250, 250]));
        ri++;
        doc.rect(10, y, pw - 20, 5, 'F');
        doc.setDrawColor(220, 220, 220);
        doc.rect(10, y, pw - 20, 5, 'S');
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      caseId: selectedData.child_caseid,
      remarks: remarks,
      username: userToken?.username,
     empname: userToken?.employee
    ...
    try {
const response = await fetch(
  `${API_HRM_PROCESS}/Note-For-AprvlUpdt`,
  {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${userToken.token}`,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100 shadow-sm">
              <DetailRow label="Name" value={selectedData?.name} />
              ...
              <DetailRow label="DEPT" value={selectedData?.DEPT} />
...
/>
                ...
                <DetailRow label="Offered Designation" value={selectedData?.DESIG ||  selectedData?.MANPOWER_DESG ||  'N/A'} />
