...
const { serviceDate } = location.state || {};
  ...
  const fetchMaintenanceFiles = async () => {
    ...
    setLoading(true);
    try {
      console.log('Fetching maintenance files for vehicle:', vehicleNumber);

      const response = await axios.get(
        `${API_BASE_URL}maint-files/${vehicleNumber}`,
        {
          headers: {
      ...
      if (responseFiles.length === 0) {
        setError(`No maintenance files found for vehicle: ${vehicleNumber}`);
      } else {
        setError(null);
      }
      ...
      setError(error.response?.data?.message || 'Failed to fetch maintenance files');
      setFiles([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="60vh">
        ...
        <Typography variant="h6" ml={2}>Loading maintenance files...</Typography>
      </Box>
    );
  }
              ...
              <div className="flex-grow flex justify-center">
                <div className="bg-[#5ea8bb] px-5 py-1.5 rounded-lg inline-block -ml-20">
                  <h1 className="text-xl font-bold text-white">
                    Maintenance Files - {vehicleNumber}
                  </h1>
                </div>
              </div>
              <button
            ...
            {files.length === 0 && !error && !loading ? (
              <Paper sx={{ padding: 4, textAlign: 'center' }}>
                ...
                <Typography variant="body2" color="textSecondary">
                  No maintenance files are available for vehicle: <strong>{vehicleNumber}</strong>
                </Typography>
              </Paper>
            ) : (
                      ...
                      <CardContent sx={{ flexGrow: 1, p: 2 }}>
                        <Box display="flex" alignItems="center" mb={1}>
                          ...
                          <Typography variant="subtitle1" fontWeight="bold">
                            {file.fileName || `Maintenance File ${index + 1}`}
                          </Typography>
                        </Box>
  ...
  <strong>Vehicle:</strong> {file.vehicleNumber}/



  {formatDate(file.serviceDate)}
</Typography>
                      ...
                      <CardActions sx={{ justifyContent: 'center', pt: 0.5, pb: 1 }}>
                        <Button
                          ...
                          onClick={() => handleFileView(file.filePath, file.fileName)}
                          sx={{
...
export default ViewallMaintFiles;
