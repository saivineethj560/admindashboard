...
const TaxDisplay = () => {
  ...
  const exportToExcel = () => {
    try {
      ...
      const exportData = filteredData.map((row, index) => ({
        ...
        'TAX START DATE': formatDate(row.TAX_START_DATE),
         'TAX REMAINDER DATE':formatDate(row.TAX_REMAINDER_DATE),
        'COST':row.COST ||'',
      ...
      const filename = `Tax_Report_${currentDate}.xlsx`;
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}getTaxData`, {
        headers: {
  ...
  const columns = [
    ...
    { 
     ...
     { 
      field: 'TAX_REMAINDER_DATE', 
      headerName: 'TAX REMAINDER DATE', 
      flex: 1.5, 
      minWidth: 160,
      headerClassName: 'header-style',
      headerAlign: 'center',
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex-grow flex justify-center">
                <div className="bg-[#5ea8bb] px-5 py-1.5 rounded-lg inline-block -ml-20">
                  <h1 className="text-xl font-bold text-white">Four Wheeler Tax Display</h1>
                </div>
              </div>
              <button
                onClick={() => navigate(-1)}
