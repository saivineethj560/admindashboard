...
const ModifyScrapPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    tagBank: '',
    tagRegMobile: '',
    // New fields for previous amounts
    prevFastAmt: '',
    prevChallanAmt: '',
  });
      ...
      if (result.status && result.data && Array.isArray(result.data)) {
        ...
        if (vehicleData) {
          ...
          setFormData({
            ...
            challanAmt: '', // Always empty for new entry
            fastAmt: '', // Always empty for new entry
            ...
            // Show current database values as "previous"
            prevFastAmt: vehicleData.FAST_AMT || '0',
            prevChallanAmt: vehicleData.CHALLAN_AMT || '0',
          });
  ...
  const handleConfirmedSubmit = async () => {
    ...
    try {
      ...
      // Add previous amounts for reference (these won't be updated in DB, just for tracking)
      formDataToSend.append('prevFastAmt', formData.prevFastAmt);
      formDataToSend.append('prevChallanAmt', formData.prevChallanAmt);
      ...
      const response = await fetch(`${API_BASE_URL}update-fasttag`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${userToken.token}`,
          'Accept': 'application/json',
  ...
  const readOnlyInputStyle = "w-full border border-gray-300 rounded-full px-2 py-1 text-sm bg-gray-100 cursor-not-allowed text-gray-600";
              ...
              <div className="flex-grow flex justify-center">
                <div className="bg-[#5ea8bb] px-4 py-1 rounded-lg inline-block -ml-20">
                  <h1 className="text-lg font-bold text-white">
                    {formData.vehicleNumber ? `Edit Fast Tag / Challan - ${formData.vehicleNumber}` : 'Fast Tag / Challan Change Tab'}
                  </h1>
                </div>
              </div>
              <div>
                ...
                {showFastTagSection && (
                  <div className="mt-4 border-2 border-blue-500 rounded-lg p-3 shadow-sm bg-blue-50 transform transition-all duration-300 ease-in-out">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      {/* Previous Fast Tag Amount - READ ONLY */}
                      {/* <div>
                        <label className="block text-indigo-800 font-bold text-xs mb-0.5">
                          Previous Fast Tag Amt
                        </label>
                        <input
                          type="text"
                          name="prevFastAmt"
                          value={`₹ ${formData.prevFastAmt || '0'}`}
                      ...
                      {/* Fast Tag Amount - EDITABLE */}
                      <div>
                        <label className="block text-indigo-800 font-bold text-xs mb-0.5">
                          New Fast Tag Amt<span className="text-red-500 ml-1">*</span>
                        </label>
                        <input
                          type="number"
                          name="fastAmt"
                ...
                {showChallanSection && (
                  <div className="mt-4 border-2 border-red-500 rounded-lg p-3 shadow-sm bg-red-50 transform transition-all duration-300 ease-in-out">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      <div>
                        <label className="block text-indigo-800 font-bold text-xs mb-0.5">
                          Check Date <span className="text-red-500 ml-1">*</span>
                        </label>
                        <input
                          type="date"
                          name="trafDt"
                      ...
                      {/* Previous Challan Amount - READ ONLY */}
                      <div>
                        <label className="block text-indigo-800 font-bold text-xs mb-0.5">
                          Previous Challan Amt
                        </label>
                        <input
                          type="text"
                          name="prevChallanAmt"
                          value={`₹ ${formData.prevChallanAmt || '0'}`}
                          className={readOnlyInputStyle}
                          readOnly
                          placeholder="Previous amount"
                        />
                      ...
                      {/* Challan Amount - EDITABLE */}
                      <div>
                        <label className="block text-indigo-800 font-bold text-xs mb-0.5">
                           Challan Amount <span className="text-red-500 ml-1">*</span>
                        </label>
                        <input
                          type="number"
                          name="challanAmt"
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          accept=".pdf,.jpg,.jpeg,.png"
                          onChange={handleInputChange}
                          className="block w-full text-xs text-gray-700 border border-red-500 rounded-full cursor-pointer file:mr-2 file:py-0.5 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100"
                        />
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="text-center">
              ...
              {/* Display amount changes if any */}
              {(formData.fastAmt || formData.challanAmt) && (
                <div className="bg-gray-50 rounded-lg p-3 mb-4 text-left">
                  <h4 className="font-medium text-gray-700 mb-2">Amount Changes:</h4>
                  {formData.fastAmt && (
                    <div className="text-sm text-gray-600 mb-1">
                      FastTag: ₹{formData.prevFastAmt || '0'} → ₹{formData.fastAmt}
                    </div>
                  )}
                  {formData.challanAmt && (
                    <div className="text-sm text-gray-600">
                      Challan: ₹{formData.prevChallanAmt || '0'} → ₹{formData.challanAmt}
                    </div>
                  )}
                </div>
              )}
...
export default ModifyScrapPage;