...
//         doc_name: apiData.doc_name || "", // 👈 Add this
//         doc_url: apiData.doc_url || "",   // 👈 Add this

//         items: apiData.items || []               // Items are inside the data object
//       });
//       // 2. ADD THIS LINE: Select all rows by default
//       if (apiData.items && apiData.items.length > 0) {
//           const allIndices = apiData.items.map((_, index) => index);
...
//                 className="flex items-center gap-2 px-3 py-1 text-[11px] font-bold text-blue-700 bg-blue-50 border border-blue-300 rounded-md hover:bg-blue-100 transition-all shadow-sm"
//             >
...
//                 View Attachment
//             </a>
//             <span className="text-[10px] text-gray-500 truncate max-w-[150px]" title={formData.doc_name}>
//                 ({formData.doc_name})
//             </span>
//         </div>
//     ) : (
//         <input type="text" value="No Attachment" readOnly className={readonlyStyle} />
//     )}
// </div>
//                 </div>
//               </div>
...
//                onItemChange={handleItemChange}   // 👈 ADD THIS on 04-03-2026------------------------------
//             />
...
const MaterialMasterApproval = () => {
  ...
  // State to track original values to detect if price was modified
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const mappedData = {
            ...
            emp_id: apiData.emp_id || "",
            designation: apiData.desig || "",
            department: apiData.department || "",
            email: apiData.email || "",
            plant: apiData.plant || "",
            profit_center: apiData.profit_center || "",
            request_type: apiData.req_type || "",
            cur_status: apiData.cur_status || "",
            cur_task: apiData.cur_task || "",
            cur_usr: apiData.cur_usr || "",
            doc_name: apiData.doc_name || "",
            doc_url: apiData.doc_url || "",
            items: apiData.items || []
          };

          setFormData(mappedData);
          
          // Deep copy items to track original state for price comparison
          setOriginalItems(JSON.parse(JSON.stringify(apiData.items || [])));

          // Select all items by default
          if (apiData.items && apiData.items.length > 0) {
            setSelectedRows(apiData.items.map((_, index) => index));
          }
        } else {
  ...
  const handleSelectAll = (e) => {
    setSelectedRows(e.target.checked ? formData.items.map((_, index) => index) : []);
  };
  ...
  const handleAction = (status) => {
    ...
    // 1. Check if price has changed compared to original load
    const hasPriceChanged = formData.items.some((item, index) => {
        const originalPrice = parseFloat(originalItems[index]?.PRICE || 0);
        const currentPrice = parseFloat(item.PRICE || 0);
        return originalPrice !== currentPrice;
    });

    // 2. Comments Mandatory Logic
    if (status === 'Approved') {
        // Must provide comments if price changed
        if (hasPriceChanged && !approvalComments.trim()) {
            Swal.fire("Comments Required", "Price has been modified. Please provide a justification in the comments.", "warning");
            return;
        }
        // Must provide comments if items were unselected (Partial Approval)
        if (selectedRows.length < formData.items.length && !approvalComments.trim()) {
            Swal.fire("Comments Required", "Some items are unselected. Please provide a reason in the comments.", "warning");
            return;
        }
    }

    // Must provide comments if Rejected
    if (status === 'Rejected' && !approvalComments.trim()) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setIsProcessing(true);
        const apiEndpoint = requestType === 'Creation' 
            ? `${API_BASE_URL}material-creation-approval-submit` 
            : `${API_BASE_URL}material-extension-approval-submit`;
        ...
        try {
          ...
          const res = await response.json();
          if (res.success) {
            await Swal.fire('Success', `Request has been ${status.toLowerCase()}`, 'success');
            navigate('/dashboard');
          } else {
    ...
    <div className="min-h-screen px-4 py-3 bg-gray-50">
      <div className="mx-auto max-w-7xl">
        <div className="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-lg">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200">
            <img src="../mat_mastquote.png" alt="Logo" className="object-contain h-10 w-38" />
            <div className="px-6 py-1 bg-indigo-900 rounded-lg shadow-md">
              <h1 className="text-base font-bold tracking-wide text-white uppercase">Material Approval Portal</h1>
            </div>
            <button onClick={() => navigate(-1)} className="flex items-center gap-1 bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-700">
              <HiArrowLeft size={16} /> Back
            </button>
          </div>
          ...
          <div className="p-4">
            <div className="grid grid-cols-1 gap-4 mb-4 lg:grid-cols-5">
              <div className="flex items-center justify-center p-3 border border-blue-100 shadow-inner lg:col-span-1 bg-gradient-to-b from-blue-50 to-white rounded-xl min-h-[160px]">
                <img src="../mat_masterimg.png" alt="Icon" className="object-contain w-full h-full" />
              </div>

              <div className="space-y-2 lg:col-span-4">
                <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                  <div className="flex items-center"><label className={labelStyle}>Date</label><input type="text" value={formData.date} readOnly className={readonlyStyle} /></div>
                  ...
                  <div className="flex items-center"><label className={labelStyle}>Name</label><input type="text" value={formData.name} readOnly className={readonlyStyle} /></div>
                  <div className="flex items-center"><label className={labelStyle}>Department</label><input type="text" value={formData.department} readOnly className={readonlyStyle} /></div>
                  <div className="flex items-center"><label className={labelStyle}>Plant</label><input type="text" value={formData.plant} readOnly className={readonlyStyle} /></div>
                  <div className="flex items-center"><label className={labelStyle}>Request Type</label><input type="text" value={formData.request_type} readOnly className={readonlyStyle} /></div>
                  <div className="flex items-center">
                    <label className={labelStyle}>Attachment</label>
                    {formData.doc_url ? (
                        <a href={formData.doc_url} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-blue-700 bg-blue-50 px-3 py-1 border border-blue-300 rounded hover:bg-blue-100 underline flex items-center gap-1">
                          📎 View File ({formData.doc_name})
                        </a>
                    ) : <input type="text" value="No Attachment" readOnly className={readonlyStyle} />}
                  </div>
                </div>
            ...
            <div className="p-3 mt-4 bg-white border border-gray-200 rounded-lg shadow-sm">
              <label className="block mb-1 text-[11px] font-bold text-indigo-900 uppercase">
                {formData.cur_task} REMARKS / COMMENTS
              </label>
              <textarea
                rows="2"
                className="w-full p-2 text-sm border border-blue-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400"
            ...
            <div className="flex justify-center gap-4 mt-4">
              <button onClick={() => handleAction('Approved')} disabled={isProcessing} className="flex items-center gap-2 px-10 py-2.5 text-sm font-bold text-white uppercase rounded-full bg-emerald-600 hover:bg-emerald-700 shadow-lg disabled:opacity-50 transform hover:scale-105 transition-all">
                <HiCheck size={18} /> Approve
              </button>
              <button onClick={() => handleAction('Rejected')} disabled={isProcessing} className="flex items-center gap-2 px-10 py-2.5 text-sm font-bold text-white uppercase rounded-full bg-rose-600 hover:bg-rose-700 shadow-lg disabled:opacity-50 transform hover:scale-105 transition-all">
                <HiX size={18} /> Reject
              </button>
            </div>
          </div>
