...
import ScrapApprovalTable from "./ScrapApprovalTable";
import ScrapUnassignTable from "./ScrapUnassignTable";

const ScrapSaleApprovalForm = () => {
    ...
    const fetchRequestDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}scrap-request-details/${case_id}`,
          {
            headers: {
  ...
  if (currentTask === "PURCHASE") {
    // REQUIREMENT: Send ALL line items to the backend
    // Backend will check if SO numbers are present to decide "Completed" vs "TO_DO"
    selectedItems = formData.items; 
    unselectedItems = [];
    
    // VALIDATION: We removed the "isAnySOMissing" check here 
    // because you mentioned it's not mandatory.
  } else {
    // Standard Flow (STORES_HEAD, VP, EVC) using checkboxes
    selectedItems = formData.items.filter((_, index) => selectedRows.includes(index));
    unselectedItems = formData.items.filter((_, index) => !selectedRows.includes(index));
  }
    // const selectedItems = formData.items.filter((_, index) =>
    //   selectedRows.includes(index),
    // );
    // const unselectedItems = formData.items.filter(
    //   (_, index) => !selectedRows.includes(index),
    // );
  
    if (selectedItems.length === 0 && formData.cur_task !='PURCHASE') {
      Swal.fire(
        "Selection Required",
    ...
    if (status === "Rejected" && !approvalComments.trim()) {
      Swal.fire(
        "Required",
        "Please provide a reason for rejection in the comments box.",
        "warning",
      );
      return;
    }
      ...
      if (result.isConfirmed) {
        ...
        try {
          if (currentTask === "STORES_HEAD")
            apiEndpoint = `${API_BASE_URL}storehead_approval_sc`;
          else if (currentTask === "VP")
            apiEndpoint = `${API_BASE_URL}vp_approval`;
          else if (currentTask === "EVC")
            apiEndpoint = `${API_BASE_URL}evc_approval_sc`;
          else if (currentTask === "PURCHASE")
            apiEndpoint = `${API_BASE_URL}purchase_approval_sc`;
          ...
          const payload = {
            ...
            VP_MAIL: formData.VP_MAIL,
            EVC_MAIL: formData.EVC_MAIL,
            PURCHASE_MAIL: formData.PURCHASE_MAIL,
  ...
  const readonlyStyle =
    "w-full border-2 border-blue-300 rounded-lg py-1.5 px-4 text-xs bg-gradient-to-r from-gray-50 to-gray-100 cursor-not-allowed text-gray-700 font-medium shadow-sm";

  if (isLoading)
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        <div className="w-12 h-12 border-b-2 border-indigo-600 rounded-full animate-spin"></div>
        <p className="italic font-semibold text-indigo-800">
      ...
      <div className="overflow-hidden bg-white border-2 border-blue-200 shadow-xl rounded-2xl">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-blue-300 bg-gradient-to-r from-blue-100 via-indigo-50 to-purple-100">
          <div className="flex items-center">
            <img
              src="../scquote.png"
              alt="Logo"
              className="object-contain h-8 transition-transform rounded hover:scale-105"
            />
          </div>
          ...
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="text-blue-500 hover:bg-blue-50 rounded-full p-1.5 border border-blue-400 transition-all hover:scale-110"
          >
            <HiArrowLeft size={18} />
          </button>
        </div>
        ...
        <div className="p-6 bg-gradient-to-b from-white to-gray-50">
          ...
          <div className="pt-2 mt-4 border-t border-gray-100">
            {formData.cur_task === "PURCHASE" ? (
              // Show this table if task is PURCHASE
              <ScrapUnassignTable
          ...
          <div className="flex justify-center gap-6 mt-8">
            {formData.cur_task === "PURCHASE" ? (
              /* --- PURCHASE TASK: ONLY SUBMIT BUTTON --- */
              <button
                onClick={() => handleAction("Approved")} // Status "Approved" triggers the SO saving in backend
                disabled={isProcessing}
                className="flex items-center gap-3 px-16 py-3 text-sm font-black tracking-widest text-white uppercase transition-all rounded-full shadow-lg bg-gradient-to-r from-indigo-600 to-blue-700 hover:shadow-xl hover:scale-105 active:scale-95 disabled:opacity-50"
              >
                <HiCheck size={20} /> Submit Request
              </button>
            ) : (
              /* --- OTHER TASKS: APPROVE & REJECT BUTTONS --- */
              <>
                <button
                  onClick={() => handleAction("Approved")}
                  disabled={isProcessing}
...
export default ScrapSaleApprovalForm;
