...
function Manapp() {
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      ...
      try {
        ...
        // ✅ Backend wraps in 'data' key → response.data.data
        ...
        if (record) {
          setFormData(prev => ({
            ...prev,
            ...record,
            caseid: record.CHILD_CASEID,
          }));

          if (record.CUR_TASK) {
            setCurrentTask(record.CUR_TASK);
          }

          // ── Populate raiser dept & designation ──
          setDeptDesign({
            empDept: record.RAISER_DEPT || '',
            empDesignation: record.RAISER_DESG || '',
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const action = e.nativeEvent?.submitter?.value
      || e.nativeEvent?.submitter?.getAttribute('data-value')
      || 'Approve';
    ...
    if (action === "Reject") {
      const rejectResult = await Swal.fire({
        ...
        text: 'Are you sure you want to reject this manpower request? This action cannot be undone.',
        icon: 'warning',
        showCancelButton: true,
      ...
      try {
        ...
        switch (currentTask) {
          case "GM": role = "GM"; endpoint = `manpower-gm-data/${case_id}`; break;
          case "PRJ_HEAD": role = "PRJ_HEAD"; endpoint = `manpower-prj-data/${case_id}`; break;
          case "FUNC_HEAD": role = "FUNC_HEAD"; endpoint = `manpower-func-data/${case_id}`; break;
          case "SP": role = "SP"; endpoint = `manpower-sp-data/${case_id}`; break;
          case "EVC": role = "EVC"; endpoint = `manpower-evc-data/${case_id}`; break;
          case "HOD": role = "HOD"; endpoint = `manpower-hod-data/${case_id}`; break;
          default: role = "APPROVER"; endpoint = `mrfRevertBack/${case_id}`;
        }
        ...
        if (response.status === 200 || response.status === 201) {
          Swal.fire({
            icon: 'success',
            title: 'Rejected!',
            text: `${role} has rejected the request successfully.`,
    ...
    if (action === "REVERT") {
      ...
      if (result.isConfirmed) {
        ...
        // User cancelled or closed the dialog — stay on page, no action
        return;
      }
    }
    ...
    try {
      const payload =
      {
        ...
        cur_tas: currentTask,
        department: formData.DEPT,
        designation: formData.MANPOWER_DESG,
        loc: formData.PLANT,
      };
  ...
  const revertedSubmit = async (e) => {
    ...
    const payload = {
      case_id: case_id,
      raiserremarks: formData[remarksFieldKey] || '',  // ✅ explicit key for RAISER flow
    };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-1.5">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="rdate"
                  value={formData.tdate ? new Date(formData.tdate).toLocaleDateString("en-GB") : new Date().toLocaleDateString("en-GB")}
                  readOnly
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-1.5">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="Position"
                  value={formData.POSITION || formData.Position || formData.sub_post || ''}
                  readOnly
              ...
              <>
                ...
                {currentTask !== "RAISER" && (
                  <Button
                    ...
                    style={{ backgroundColor: isLoading ? '#9ca3af' : !formData.caseid ? undefined : '#dc2626' }}
                    startIcon={isLoading ? <CircularProgress size={12} /> : <Close style={{ fontSize: '14px' }} />}>
                    Reject
                  </Button>
                )}

                {/* Revert — only for SP and EVC */}
                {(currentTask === "SP" || currentTask === "EVC") && (
                  <Button
                    ...
                    disabled={isLoading || !formData.caseid}
                    className="px-3 py-0.5 text-[10px] rounded font-medium shadow-md"
                    style={{ backgroundColor: isLoading ? '#9ca3af' : !formData.caseid ? undefined : '#ea580c' }}
                    startIcon={isLoading ? <CircularProgress size={12} /> : <Close style={{ fontSize: '14px' }} />}>
                    Revert
                  </Button>
                )}
                ...
                <Button
                  ...
                  disabled={isLoading || !formData.caseid}
                  className="px-3 py-0.5 text-[10px] rounded font-medium shadow-md"
                  style={{ backgroundColor: isLoading ? '#9ca3af' : !formData.caseid ? undefined : '#16a34a' }}
