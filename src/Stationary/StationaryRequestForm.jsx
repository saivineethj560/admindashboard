...
const StationeryRequestForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // hod_name: userToken?.StoresUserName || userToken?.DEPT_HOD,
    // hod_name: (userToken.Is_Employee=='2')?userToken?.StoresUserName : userToken?.DEPT_HOD,
    hod_name: "",
    
    DeptHODName: userToken.DeptHODName,
  ...
  // 🔥 SET HOD NAME BASED ON EMP CATEGORY ---------added this on 19112025-by vineeth
useEffect(() => {
  if (!userToken) return;
...
//------------end vineeth--------------
  ...
  const validateForm = () => {
    ...
    // For HR users, add these two fields as required
    if (userToken.Is_Employee ==0) {
      requiredFields.push('req_for_emp_id', 'req_for_emp_dept');
    }else{
      
      requiredFields = requiredFields.filter(
        f => !['req_for_emp_id', 'req_for_emp_dept'].includes(f)
      );
  ...
  const getInputStyle = (fieldName) => {
    ...
    if (['date', 'name', 'others_name', 'req_for_emp_id', 'req_for_emp_dept'].includes(fieldName)) {
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  {/* Req For EmpId field - Only for HR users */}
                  {userToken.Is_Employee == 0 && (
                    <div>
                      <div className="flex items-center">
                        <label className="w-1/3 text-indigo-800 font-bold text-xs">
                          Req For EmpId<span className="text-red-500 ml-1">*</span>
                        </label>
                        <input
                          type="text"
                          name="req_for_emp_id"
                          value={formData.req_for_emp_id}
                          onChange={handleChange}
                          className={getInputStyle('req_for_emp_id')}
                          placeholder="Enter employee ID"
                        />
                      </div>
                      {errors.req_for_emp_id && attempted && (
                        <div className="ml-1/3 pl-32 text-red-500 text-xs mt-0.5">{errors.req_for_emp_id}</div>
                      )}
                    </div>
                  )}

                  {/* Req For EmpDept field - Only for HR users */}
                  {userToken.Is_Employee == 0 && (
                    <div>
                      <div className="flex items-center">
                        <label className="w-1/3 text-indigo-800 font-bold text-xs">
                          Req. Emp Dept<span className="text-red-500 ml-1">*</span>
                        </label>
                        <select
                          name="req_for_emp_dept"
                          value={formData.req_for_emp_dept}
                          onChange={handleChange}
                          className={getInputStyle('req_for_emp_dept')}
                        >
                      ...
                      {errors.req_for_emp_dept && attempted && (
                        <div className="ml-1/3 pl-32 text-red-500 text-xs mt-0.5">{errors.req_for_emp_dept}</div>
                      )}
                    </div>
                  )}
                </div>
                  ...
                  <>
                    <div className="max-h-60 overflow-y-auto">
                      {(formData.request_for === "Standard" || formData.request_for === "Projects") ? (
                        // HR: Standard/Projects - SNO + Item + Quantity (NO Action column)
                        <div className="max-h-98 overflow-y-auto border rounded-b-md">
                          {stationeryOptions[formData.request_for].map((itemName, index) => (
                            <div
                              key={index}
                              className="grid grid-cols-12 items-center border-b border-gray-300 py-2 px-3 text-sm bg-white"
                            >
                              {/* SNO Column */}
                              <div className="col-span-1 text-center border-r-2 border-gray-300">
                                <span className="font-bold text-lg text-blue-700">{index + 1}</span>
                              ...
                              {/* Stationery Item */}
                              <div className="col-span-7 pr-3 border-r-2 border-gray-300">
                        ...
                        // HR: Other - Show all columns including SNO and Action
                        <div className="max-h-36 overflow-y-auto border rounded-b-md">
                          {formData.items.map((item, index) => (
                            <div
                              key={index}
                              className="grid grid-cols-12 items-center border-b border-gray-300 py-2 px-2 text-sm bg-blue-50"
                            >
                              {/* SNO Column */}
                              <div className="col-span-1 text-center border-r-2 border-gray-300">
                                <span className="font-bold text-lg text-blue-700">{index + 1}</span>
                              ...
                              <div className="col-span-5 pr-2 border-r-2 border-gray-300">
                                <SearchableSelect
                                  options={backendStationeryItems.map(item => item.item_name)}
                                  value={item.stationary}
                                  onChange={(value) => handleItemChange({ target: { name: 'stationary', value } }, index)}
                              ...
                              <div className="col-span-4 pl-2 border-r-2 border-gray-300">
                                <input
                                  type="number"
                                  name="quantity"
                                  placeholder="Qty"
                                  ...
                                  value={item.quantity || ''}
                                  onChange={(e) => handleItemChange(e, index)}
                                  className="w-full border-2 border-blue-500 rounded-full p-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-600 bg-blue-50 focus:bg-blue-100 shadow-sm"
                                />
                              </div>
                        ...
                        // HR: Default case - Show all columns including SNO
                        <div className="max-h-36 overflow-y-auto border rounded-b-md">
                          {formData.items.map((item, index) => (
                            <div
                              key={index}
                              className="grid grid-cols-12 items-center border-b border-gray-300 py-2 px-2 bg-white"
                            >
                              {/* SNO Column */}
                              <div className="col-span-1 text-center border-r-2 border-gray-300">
                                <span className="font-bold text-lg text-blue-700">{index + 1}</span>
