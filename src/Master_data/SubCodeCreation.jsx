...
const SubCode = () => {
    ...
    const validateChangeRows = () => {
        ...
        // ── Duplicate subcode check ──
        const comboCounts = {};
        changeRows.forEach((r, idx) => {
            const key = `${r.code}|${r.subcode}`;
            if (r.code && r.subcode) {
                if (!comboCounts[key]) comboCounts[key] = [];
                comboCounts[key].push(idx);
            }
        });
        Object.entries(comboCounts).forEach(([key, indices]) => {
            if (indices.length > 1) {
                hasError = true;
                const [code, subcode] = key.split('|');
                indices.forEach(idx => {
                    newErrors[idx] = {
                        ...newErrors[idx],
                        subcode: `Duplicate "${code} / ${subcode}" — each sub code must be unique`,
                    };
                });
            }
        });
                                            ...
                                            <tr key={i} className={`${i % 2 === 0 ? 'bg-teal-50/40' : 'bg-white'} border-t border-slate-100 hover:bg-teal-50 transition-colors`}>
                                                ...
                                                <td className="px-2 py-2 align-top">
                                                    <input
                                                        ...
                                                        className={!row.subcode ? readOnly : inpTeal(!!changeErrors[i]?.position)}
                                                    />
                                                    {changeErrors[i]?.position && (
                                                        <p className="text-red-500 text-[10px] mt-0.5">{changeErrors[i].position}</p>
                                                    )}
                                                </td>
