...
import { Search, RefreshCw, ChevronDown, ChevronUp, Layers, Users, BarChart2, Hash, ArrowLeft, Factory, Download, TrendingDown } from "lucide-react";
...
function BalanceBadge({ value, size = "md" }) {
  const hasVacancy = value > 0;
  const fullyStaffed = value === 0;
  const overStaffed = value < 0;

  const bg = fullyStaffed ? "#f0fdf4" : hasVacancy ? "#fffbeb" : "#fff1f2";
  const color = fullyStaffed ? "#15803d" : hasVacancy ? "#92400e" : "#dc2626";
  const border = fullyStaffed ? "#bbf7d0" : hasVacancy ? "#fde68a" : "#fecdd3";

  const icon = fullyStaffed ? "✓" : hasVacancy ? "▼" : "▲";
  const prefix = overStaffed ? "+" : "";
  ...
  if (size === "sm") {
    return (
      <span style={{
        ...
        {icon} {fullyStaffed ? "0" : `${prefix}${value}`}
      </span>
    );
  }
  ...
  return (
    <span style={{
      ...
      minWidth: 38, textAlign: "center", justifyContent: "center",
...
// ── Raised badge helper ────────────────────────────────────────────────────
function RaisedBadge({ value, size = "md" }) {
...
export default function MasterDataUdyan() {
  ...
  // Helper to get raised count from row (now coming directly from API)
  const getRaisedCount = (row) => {
    // For plant-specific view (when plantFilter is active)
    if (plantFilter) {
      return row.RAISED_COUNT || 0;
    }
    // For "All Plants" view - use the combined count
    return row.RAISED_COUNT_ALL || 0;
  };
  const getDisplayTotal = (row) => plantFilter ? row.TOTAL_PLAN : row.TOTAL_ALL;
  const getFilled = (row) =>
    row.TOTAL_FILLED ?? row.FILLED ?? row.ACTUAL ?? row.TOTAL_ACTUAL ?? row.HEADCOUNT ?? 0;
  const getDisplayBalance = (row) => {
    const total = getDisplayTotal(row);
    const raised = getRaisedCount(row);
    return total - raised;
  };
  ...
  // ── Summary with raised counts included ──
  const summary = useMemo(() => {
    const uniquePositions = plantFilter
      ? filtered
      : [...new Map(data.map(r => [r.CODES, r])).values()];

    const totalRaised = uniquePositions.reduce((s, r) => s + (getRaisedCount(r) || 0), 0);
    ...
    return {
      ...
      totalBalance: uniquePositions.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0),
  ...
  const downloadExcel = () => {
    const currentPlantFilter = plantFilter;
    const rows = filtered.map((row, idx) => ({
      ...
      "Total Plan": currentPlantFilter ? row.TOTAL_PLAN : row.TOTAL_ALL,
      "Filled": getFilled(row),
      "Raised": getRaisedCount(row),
      "Balance (Open)": (currentPlantFilter ? row.TOTAL_PLAN : row.TOTAL_ALL) - getFilled(row),
    }));
    ...
    rows.push({
      ...
      "Filled": filtered.reduce((s, r) => s + getFilled(r), 0),
      "Raised": filtered.reduce((s, r) => s + getRaisedCount(r), 0),
      "Balance (Open)": filtered.reduce((s, r) => s + ((currentPlantFilter ? r.TOTAL_PLAN : r.TOTAL_ALL) - getFilled(r)), 0),
    });

    const ws = XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [
      { wch: 6 }, { wch: 20 }, { wch: 10 },
      { wch: 12 }, { wch: 24 }, { wch: 36 }, { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 14 },
    ];
      ...
      <div style={{
        ...
        {!loading && !error && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {/* Raised summary pill */}
            <div style={{
              background: summary.totalRaised > 0 ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.07)",
              border: `1px solid ${summary.totalRaised > 0 ? "rgba(59,130,246,0.35)" : "rgba(255,255,255,0.14)"}`,
              borderRadius: 8, padding: "5px 12px",
              display: "flex", alignItems: "center", gap: 6
            }}>
              <span style={{ color: "#60a5fa" }}>📋</span>
              <span style={{ color: "#60a5fa", fontWeight: 700, fontSize: 15 }}>{summary.totalRaised}</span>
              <span style={{ color: "rgba(255,255,255,0.42)", fontSize: 11, fontWeight: 500 }}>Raised</span>
            </div>
            ...
            <div style={{
              ...
              border: `1px solid ${summary.totalBalance === 0 ? "rgba(52,211,153,0.35)" : summary.totalBalance > 0 ? "rgba(251,191,36,0.35)" : "rgba(248,113,113,0.35)"}`,
              borderRadius: 8, padding: "5px 12px",
              display: "flex", alignItems: "center", gap: 6
            }}>
              <TrendingDown size={12} color={summary.totalBalance === 0 ? "#34d399" : summary.totalBalance > 0 ? "#fbbf24" : "#f87171"} />
              <span style={{
                color: summary.totalBalance === 0 ? "#34d399" : summary.totalBalance > 0 ? "#fbbf24" : "#f87171",
                fontWeight: 700, fontSize: 15
              }}>
                {summary.totalBalance === 0 ? "0" : summary.totalBalance > 0 ? `${summary.totalBalance} open` : `${Math.abs(summary.totalBalance)} over`}
              </span>
              <span style={{ color: "rgba(255,255,255,0.42)", fontSize: 11, fontWeight: 500 }}>Vacancies</span>
            </div>
          </div>
        )}
      </div>
        ...
        {!loading && !error && viewMode === "grouped" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {Object.entries(grouped).map(([deptKey, grp], di) => {
              ...
              const validRows = rows.filter(r => r.POSITION && r.POSITION.trim() !== "");
              const deptTotal = validRows.reduce((s, r) => s + getDisplayTotal(r), 0);
              const deptBalance = validRows.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0);
              const deptRaised = validRows.reduce((s, r) => s + (getRaisedCount(r) || 0), 0);
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: "#64748b" }}>
                      <strong style={{ color: "#0f172a" }}>{validRows.length}</strong> positions
                    </span>
                    ...
                    <span style={{
                      display: "inline-flex", alignItems: "center", gap: 4,
                      fontSize: 12, fontWeight: 700,
                      color: deptBalance === 0 ? "#15803d" : deptBalance > 0 ? "#92400e" : "#dc2626",
                      background: deptBalance === 0 ? "#f0fdf4" : deptBalance > 0 ? "#fffbeb" : "#fff1f2",
                      border: `1px solid ${deptBalance === 0 ? "#bbf7d0" : deptBalance > 0 ? "#fde68a" : "#fecdd3"}`,
                      padding: "2px 10px", borderRadius: 20
                    }}>
                      {deptBalance === 0 ? "✓ 0" : deptBalance > 0 ? `▼ ${deptBalance} open` : `▲ ${Math.abs(deptBalance)} over`}
                    </span>
                      ...
                      >
                        <div style={{
                          ...
                          {validRows.map((row, ri) => {
                            const bal = getDisplayBalance(row) ?? 0;
                            const raised = getRaisedCount(row);
                            return (
                              ...
                              >
                                ...
                                {/* RAISED and BALANCE row - side by side */}
                                <div style={{
                                  display: "flex",
                                  gap: 12,
                                  alignItems: "center",
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                    <span style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>Raised:</span>
                                    <RaisedBadge value={raised} size="sm" />
                                  </div>

                                  {/* BALANCE / OPEN */}
                                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                    <span style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>
                                      {bal === 0 ? "Balance:" : bal > 0 ? "Open:" : "Over:"}
                                    </span>
                                    <BalanceBadge value={bal} size="sm" />
                                  </div>
                                </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr style={{
                    ...
                    {["S.No", "Plant", "Code", "Department", "Sub Code", "Position", "Total Plan", "Raised", "Balance"].map(h => (
                      <th key={h} style={{
                        padding: "9px 12px", textAlign: "left",
                        color: h === "Balance" ? "#86efac" : h === "Raised" ? "#60a5fa" : "rgba(255,255,255,0.8)",
                        fontWeight: 600,
                ...
                <tbody>
                  ...
                  {filtered.map((row, idx) => {
                    ...
                    const bal = getDisplayBalance(row) ?? 0;
                    const raised = getRaisedCount(row);
                    const totalPlan = getDisplayTotal(row);
                      ...
                      <tr key={row.SNO} className="trow" style={{ borderBottom: "1px solid #f1f5f9" }}>
                        ...
                        <td style={{ padding: "7px 12px" }}>
                          <span style={{
                            ...
                            background: totalPlan > 0 ? palette.bg : "#f8fafc",
                            color: totalPlan > 0 ? palette.accent : "#94a3b8",
                            border: `1px solid ${totalPlan > 0 ? palette.light : "#e2e8f0"}`
                          }}>{totalPlan}</span>
                        </td>
                        <td style={{ padding: "7px 12px" }}>
                          <RaisedBadge value={raised} size="md" />
                        </td>
                        <td style={{ padding: "7px 12px" }}>
                          <BalanceBadge value={bal} size="md" />
                        </td>
                      </tr>
                    );
                  })}
                    ...
                    <tr style={{
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{
                          ...
                          <span style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>{filtered.length}</span>
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{
                          ...
                          <span style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>
                            {filtered.reduce((s, r) => s + getDisplayTotal(r), 0)}
                          </span>
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{
                          display: "inline-flex", alignItems: "center", gap: 5,
                          background: filtered.reduce((s, r) => s + getRaisedCount(r), 0) > 0 ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.07)",
                          border: `1px solid ${filtered.reduce((s, r) => s + getRaisedCount(r), 0) > 0 ? "rgba(59,130,246,0.35)" : "rgba(255,255,255,0.14)"}`,
                          borderRadius: 20, padding: "3px 10px"
                        }}>
                          <span style={{ color: "#60a5fa" }}>📋</span>
                          <span style={{ color: "#60a5fa", fontWeight: 700, fontSize: 13 }}>
                            {filtered.reduce((s, r) => s + getRaisedCount(r), 0)}
                          </span>
                          <span style={{ color: "rgba(255,255,255,0.45)", fontSize: 10 }}>raised</span>
                        </span>
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{
                          display: "inline-flex", alignItems: "center", gap: 5,
                          background: filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) === 0
                            ? "rgba(52,211,153,0.15)"
                            : filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) > 0
                              ? "rgba(251,191,36,0.15)"
                              : "rgba(248,113,113,0.15)",
                          border: `1px solid ${filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) === 0 ? "rgba(52,211,153,0.35)" : filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) > 0 ? "rgba(251,191,36,0.35)" : "rgba(248,113,113,0.35)"}`,
                          borderRadius: 20, padding: "3px 10px"
                        }}>
                          <TrendingDown size={11} color={
                            filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) === 0 ? "#34d399" :
                              filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) > 0 ? "#fbbf24" : "#f87171"
                          } />
                          <span style={{
                            color: filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) === 0 ? "#34d399" :
                              filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0) > 0 ? "#fbbf24" : "#f87171",
                            fontWeight: 700, fontSize: 13
                          }}>
                            {(() => {
                              const totalBal = filtered.reduce((s, r) => s + (getDisplayBalance(r) ?? 0), 0);
                              return totalBal === 0 ? "0" : totalBal > 0 ? `${totalBal} open` : `${Math.abs(totalBal)} over`;
                            })()}
                          </span>
                          <span style={{ color: "rgba(255,255,255,0.45)", fontSize: 10 }}>vacancies</span>
                        </span>
                      </td>
                    </tr>
                  </tfoot>
