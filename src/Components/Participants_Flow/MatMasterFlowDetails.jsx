...
// const MatMasterFlowDetails = ({ flowLoading, flowData, formatDate, getStatusColor }) => {
//     if (flowLoading) {
//         return (
...
//     const itemsArray = data.mmcreation_items || [];
...
//     const gridConfig = "1.5fr 1.5fr 50px 70px 65px";
...
//                                             {item.MAT_DESCP}
//                                         </Typography>
//                                         <Typography sx={{ fontSize: '8px' }} align="center">{item.PLANT || 'N/A'}</Typography>
...
// export default MatMasterFlowDetails;
...
const MatMasterFlowDetails = ({ flowLoading, flowData, formatDate, getStatusColor }) => {
    ...
    const data = flowData.data;
    const itemsArray = data.mmcreation_items || data.mmextension_items || [];
    ...
    // HELPER: Improved to handle Partial Approvals
    const getStepInfo = (stepId, nameKey, statusKey, dateKey) => {
        // 1. Get all items that have a status for this specific step
        const stepItems = itemsArray.filter(item => item[statusKey] && item[statusKey] !== "PENDING");
        
        if (stepItems.length === 0) return { name: null, status: null, date: null };

        // 2. Logic: If ANY item in this step is 'Approve', the Step Header should show 'Approve'
        const hasApprove = stepItems.some(item => item[statusKey] === 'Approve');
        
        // 3. Determine the display status
        // We prioritize 'Approve' from items over the global status because global status 
        // often just reflects the status of the first or last item processed.
        const finalStatus = hasApprove ? 'Approve' : (data[statusKey] || stepItems[0][statusKey]);
    ...
    // Updated Grid Config to 4 columns: Description, Plant, Price, Status
    const gridConfig = "2fr 1.5fr 60px 80px";
        ...
        return (
            <Box sx={{ border: '1px solid #ccc', borderRadius: '6px', p: 1, mb: 1, backgroundColor: displayStatus === 'TO_DO' ? '#fffde7' : '#f5f5f5' }}>
                <Box sx={{ display: 'flex', gap: 1 }}>
                    ...
                    {/* Right side: Items Table */}
                    <Box sx={{ flex: 1.5, border: '1px solid #ddd', borderRadius: '4px', p: 1, backgroundColor: 'white' }}>
                        <Box sx={{ display: 'grid', gridTemplateColumns: gridConfig, gap: '8px', borderBottom: '1px solid #ddd', pb: 0.5, px: 0.5 }}>
                            <Typography variant="caption" fontWeight="bold" sx={{ fontSize: '10px' }}>Description</Typography>
                            <Typography variant="caption" fontWeight="bold" align="left" sx={{ fontSize: '10px' }}>Plant</Typography>
                            <Typography variant="caption" fontWeight="bold" align="center" sx={{ fontSize: '10px' }}>Price</Typography>
                            <Typography variant="caption" fontWeight="bold" align="center" sx={{ fontSize: '10px' }}>Status</Typography>
                        </Box>
                        ...
                        <Box sx={{ maxHeight: '150px', overflowY: 'auto' }}>
                            {visibleItems.map((item, index) => {
                                const itemDisplayStatus = item[itemStatusKey] || item.CUR_STATUS;
                                return (
                                    <Box key={index} sx={{ display: 'grid', gridTemplateColumns: gridConfig, gap: '8px', py: 0.5, px: 0.5, borderBottom: '1px solid #eee', alignItems: 'center' }}>
                                        <Typography sx={{ fontSize: '9px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={item.MAT_DESCP}>
                                            {item.MAT_DESCP}
                                        </Typography>
                                        <Typography sx={{ fontSize: '8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} align="left">
                                            {item.PLANT || 'N/A'}
                                        </Typography>
                                        <Typography sx={{ fontSize: '9px' }} align="center">{item.PRICE || 0}</Typography>
                                        <Box sx={{ 
    ...
    const isOverallTodo = itemsArray.some(item => item.CUR_STATUS === 'TO_DO');
    // Footer Logic: Find the last item that isn't pending to determine the summary status
    const lastProcessedItem = [...itemsArray].reverse().find(i => i.CUR_STATUS !== 'PENDING' && i.CUR_STATUS !== 'SUBMITTED');
    ...
    return (
        <Box sx={{ p: 1 }}>
            ...
            {/* Footer Summary Box */}
            <Typography sx={{ textAlign: 'center', color: '#1976d2', my: 0.2, lineHeight: 1 }}>↓</Typography>
            <Box sx={{ border: '1px solid #ccc', borderRadius: '6px', p: 1, backgroundColor: isOverallTodo ? '#c6ddf1' : '#e8f5e9', textAlign: 'center' }}>
                <Typography variant="body2" fontWeight="bold" sx={{ fontSize: "12px" }}>
                    Current User: {itemsArray.find(i => i.CUR_STATUS === 'TO_DO')?.CUR_USR || lastProcessedItem?.CUR_USR || data.current_user}
                </Typography>
                <Typography variant="body2" fontWeight="bold" sx={{ mt: 0.5, color: getSafeStatusColor(isOverallTodo ? "TO_DO" : lastProcessedItem?.CUR_STATUS || "Reject"), fontSize: "12px" }}>
